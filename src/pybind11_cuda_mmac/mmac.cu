...
__global__ void kernal_mmac(int* A, int* W, int* C, const int M, const int N, const int K);
...
void cu_mmac(int* A, int* W, int* C, const int M, const int N, const int K)
{
	...
	dim3 grid;
	grid.x = (N + blk.x - 1) / blk.x;
	grid.y = (M + blk.y - 1) / blk.y;
	grid.z = 1;
	...
	kernal_mmac << < grid, blk >> > (d_a, d_w, d_c, M, N, K);
